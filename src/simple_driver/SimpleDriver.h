...
public:
	...
	virtual sigevo_champ::CarControl wDrive(sigevo_champ::CarState cs);
...
private:
	...
	int getGear(sigevo_champ::CarState &cs);

	// Solves the steering subproblems
	float getSteer(sigevo_champ::CarState &cs);
	
	// Solves the gear changing subproblems
	float getAccel(sigevo_champ::CarState &cs);
	
	// Apply an ABS filter to brake command
	float filterABS(sigevo_champ::CarState &cs,float brake);

	// Solves the clucthing subproblems
	void clutching(sigevo_champ::CarState &cs, float &clutch);
};
