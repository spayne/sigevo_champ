...
const float SimpleDriver::maxSpeedDist=0;
const float SimpleDriver::maxSpeed=80;
...
static sigevo_champ::CarControl make_cc(float accel, float brake, float gear, float steer, float clutch)
{
...
int
SimpleDriver::getGear(sigevo_champ::CarState &cs)
{
...
float
SimpleDriver::getSteer(sigevo_champ::CarState &cs)
{
...
float
SimpleDriver::getAccel(sigevo_champ::CarState &cs)
{
...
sigevo_champ::CarControl
SimpleDriver::wDrive(sigevo_champ::CarState cs)
{
    ...
    if (stuck > stuckTime) 
    {
        ...
        sigevo_champ::CarControl cc = make_cc(1.0,0.0,gear,steer,clutch);
        return cc;
    }
    ...
    else // car is not stuck
    {
        ...
        // build a CarControl variable and return it
        sigevo_champ::CarControl cc = make_cc(accel,brake,gear,steer,clutch);
        return cc;
    }
}

float
SimpleDriver::filterABS(sigevo_champ::CarState &cs,float brake)
{
...
void
SimpleDriver::clutching(sigevo_champ::CarState &cs, float &clutch)
{
  double maxClutch = clutchMax;

  // Check if the current situation is the race start
  if (cs.curLapTime<clutchDeltaTime  && cs.stage==sigevo_champ::CarState::RACE_STAGE &&
      cs.distRaced<clutchDeltaRaced)
    clutch = maxClutch;
  ...
  if(clutch > 0)
  {
	...
	clutch = std::min(maxClutch,double(clutch));

	// if clutch is not at max value decrease it quite quickly
	if (clutch!=maxClutch)
	{
	  clutch -= delta;
	  clutch = std::max(0.0,double(clutch));
	}
