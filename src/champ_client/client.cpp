...
Comment by Sean Payne Mar-May 2011:

All of the code in this file is from the SIG EVO Torcs client code -
I made only minimal changes so that it would work with my simpledriver:

* init_0 is called to initialize the driver with argc, argv - this wasn't
  supported in the original interface

* the code now exits if a ctrl-c is received during select - otherwise
...
//
// SDP Mar 14, 2011: Define the type and include file for the single driver
//                   the main code below instantiates 
#define __DRIVER_CLASS__ RosDriver
#define __DRIVER_INCLUDE__ "RosDriver.h" 
#define DRIVER_IS_ROS
    ...
    do
    {
        ...
        ***********************************************************************************/
		...
		fprintf(stderr, "errno: %d\n", errno);
            ...
            int retval = select(socketDescriptor+1, &readSet, NULL, NULL, &timeVal);
            if (retval>0)
            {
		...
		if ( (++currentStep) != maxSteps)
		{
			...
			sprintf (buf, "(meta 1)");
                ...
                if (sendto(socketDescriptor, buf, strlen(buf)+1, 0,
                           ...
                           sizeof(serverAddress)) < 0)
                {
                    cerr << "cannot send data - skipping";
		/*	
                    CLOSE(socketDescriptor);
                    exit(1);
		*/
